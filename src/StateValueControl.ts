...
export class StateValueControl {
    /**
    ...
    constructor() {
        ...
        this._errorPane = $("<div>").addClass("errorPane").appendTo(this.statecontainerElement);
        var inputs: IDictionaryStringTo<string> = initialConfig.witInputs;

        this.statefieldName = inputs["StateName"];
        // if (!this.statefieldName) {
        //     this.showError("statefieldName input has not been specified");
        // }
        ...
        this.countryName = inputs["FieldName"];
        console.log("Checking field name"+this.countryName);
    }
    ...
    protected flush(): void {
        ...
        WitService.WorkItemFormService.getService().then(
            (service: WitService.IWorkItemFormService) => {
                service.setFieldValue(this.statefieldName, this.getValue()).then(
                    (values) => {
        ...
        WitService.WorkItemFormService.getService().then(
            (service) => {
                service.getFieldValues([this.statefieldName]).then(
                    ...
                    () => {
                        this.showError("Error loading values for field: " + this.statefieldName);
                    }
                );
            }
        );
